...
tag @e[type=fs:game,scores={music_track=4},tag=!is_loop] add same_track
# STOP JUKEBOX:
execute as @e[type=fs:game,tag=!same_track] run function music/stop/all
# SETUP TRACK:
scoreboard players set @e[type=fs:game,tag=!same_track] music_track 4
...
# PLAY TRACK:
execute as @e[type=fs:game,tag=!same_track] run playsound fs.music.music_boss_rocksteady_loop @a[scores={s_music_enabled=1}]